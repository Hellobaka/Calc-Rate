...
using me.cqp.luohuaming.Calculator.Core;
using me.cqp.luohuaming.Calculator.Core.Domain;
using me.cqp.luohuaming.Calculator.Sdk.Cqp;
using me.cqp.luohuaming.Calculator.Sdk.Cqp.Enum;
using me.cqp.luohuaming.Calculator.Sdk.Cqp.EventArgs;
using me.cqp.luohuaming.Calculator.Sdk.Cqp.Interface;
using me.cqp.luohuaming.Calculator.Sdk.Cqp.Expand;
using me.cqp.luohuaming.Calculator.Sdk.Cqp.Model;
using Unity;
using Unity.Injection;

namespace me.cqp.luohuaming.Calculator.App.Export
{
	/// <summary>	
	/// 表示酷Q事件导出的类	
	/// </summary>	
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.Calculator";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.Calculator", 1, 9, "水银计算器", "1.0.1", 1, "落花茗", "定制项目，四季 2021.10.1", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.Calculator", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.Calculator", AppData.CQLog);	
